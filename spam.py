...
{m} [{k} /{m} ]{p} Author :{k} Marshallsetian
{m} [{k} \{m} ]{p} Github :{a} https://github.com/marshallsetian
'''
def wangsaf():
	os.system('clear'); print(banner)
	try:
		global target
		target = int(input(f'{m} [{h} ?{m} ]{p} nomer targetmu cok :{k} '))
	except ValueError:
...
def start():
	...
	sponge = input(f'\n{m} [{h} ?{m} ]{p} pilih 1 atau 2 ?:{h} ')
	if sponge == '1' or sponge == '01':
		wangsaf()
	elif sponge == '2' or sponge == '02':
		semes()
	else:
		exit(f'{m} [{p} !{m} ]{p} Pilih Yang Bener lah cok...')
